...
const taskWords = (task: BackgroundTask): string =>
	task.description.trim() === ""
		? task.kind
		: `${task.kind}: ${task.description.trim()}`;
...
export const SessionStandingBar = ({
	...
	const share =
		standing.usage === undefined ? undefined : cacheShare(standing.usage);
	...
	return (
		<div className="flex min-w-0 shrink-0 items-center gap-2 border-t border-border px-4 py-1.5 font-mono text-2xs text-muted-foreground">
			{standing.state === undefined ? (
				...
				<Badge variant={STATE[standing.state]}>
					{stateWords[standing.state]}
				</Badge>
			)}
			{share === undefined ? null : (
				<span className="shrink-0 font-medium text-foreground">
					{Math.round(share * 100)}% cache
				</span>
			)}
			<Background tasks={standing.background} />
			<span className="min-w-0 flex-1 truncate text-right">
				{standing.usage === undefined
					? "no usage reported yet"
					: usageLabel(standing.usage)}
			</span>
		</div>
	);
};
