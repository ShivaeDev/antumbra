...
import type { BackendFailure, OpenSessionOptions, SessionHandle, SessionInput } from "@antumbra/runner-ports/backend.ts";
...
const deliver = (session: PiSession, delivery: "followUp" | "steer") => (text: string) => session.prompt(text, delivery);

const liveSession = (runtime: PiRuntime, request: Parameters<PiRuntime["open"]>[0]) =>
	Effect.acquireRelease(runtime.open(request), (session) => session.abort.pipe(Effect.andThen(session.dispose)));
