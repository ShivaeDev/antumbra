...
A change that moves on after its description is written takes the description with it. The reviewer reads the body against the diff as it is now, so a
body that tells an earlier diff misleads them. After a push that changes what the pull request does, read the title and body again and update what no
longer holds.
