import { anyOf, applications, fence, files, importFrom, modules, packages, sanctioned, workspaceExcept } from "#boundaries/dsl.ts";
import type { BoundaryRule } from "#boundaries/model.ts";

export const ownershipPolicy = [
	fence("electron-only-in-desktop")
		.because("Only the desktop shell touches Electron APIs. Core packages stay host-agnostic.")
		.forbidsImportsFrom(packages.all)
		.to(modules.named("electron"))
		.demonstratedBy({
			illegal: importFrom(files.inPackage("domain", "src/domain.ts")).to(files.module("electron")),
	...
	fence("nothing-imports-desktop")
		...
		.to(applications.all)
		.demonstratedBy({
			illegal: importFrom(files.inPackage("domain", "src/domain.ts")).to(files.inApplication("desktop", "src/main.ts")),
			legal: importFrom(files.inPackage("domain", "src/domain.ts")).to(files.inPackage("plugin-api", "src/backend.ts")),
		}),
