...
preserve that identity and the entry cursors. The log uses SQLite's `user_version` to record completed upgrades. Its first upgrade replaces DDL-hash
metadata with a stored identity and ensures the request index without replacing the entries table. Files predating stored identity retain their
configured log id. Previously retained rollover files remain separate logs with their own identities and cursors; reading one requires its original
configured id when the file predates stored identity. Transport replies acknowledge operations; they do not fabricate Session completion. Image and
Artifact bytes live in app-managed custody, while journal rows hold their identity, ordering, and delivery or landing evidence.
...
boot, on dirty keys, at the next due time a reconciler reports from its read, and a minute after a run fails; the app refreshes them on runner
reconnect and owns any remaining cadence.
