...
export const directServiceDefinitions = (
...
export const invalidServiceMembers = (
	...
	config.properties.filter(
		(member) =>
			...
			!CONFIG_MEMBERS.has(member.name.text),
	);
...
export const returnedServiceMethods = (
	value: ts.Expression,
...
export const isEffectVoid = (
	value: ts.Expression,
	effects: ReadonlySet<string>,
): boolean =>
	ts.isPropertyAccessExpression(value) &&
	ts.isIdentifier(value.expression) &&
	effects.has(value.expression.text) &&
	value.name.text === "void";

export const isOperationReference = (
	...
	return (
		...
		imports.has(unwrapExpression(value.expression).getText())
	);
};
...
export const implementationBodies = (
	source: ts.SourceFile,
