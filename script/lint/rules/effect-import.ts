...
export const effectModule = (node: ts.Node): string | undefined => {
	...
	return module !== undefined && ts.isStringLiteral(module)
		? module.text
		: undefined;
};

const importedEffectExport = (
	identifier: ts.Identifier,
	checker: ts.TypeChecker,
	exportName: string,
): "named" | "namespace" | undefined => {
	for (const declaration of checker.getSymbolAtLocation(identifier)
		?.declarations ?? []) {
		...
		if (ts.isImportSpecifier(declaration)) {
			...
			if (
				imported === exportName &&
				(module === "effect" || module === `effect/${exportName}`)
			) {
				return "named";
			}
		}
...
export const referenceTail = (node: ts.TypeReferenceNode): string =>
	node.typeName.getText().split(".").at(-1) ?? "";

export const isEffectTypeReference = (
	...
	return (
		imported !== undefined &&
		(imported === "named" && node.typeName === identifier
			? true
			: referenceTail(node) === exportName)
	);
};
