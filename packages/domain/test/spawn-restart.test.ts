...
const blockFirstOpen = (backend: AgentBackend, opening: Deferred.Deferred<void>, first: Ref.Ref<boolean>): AgentBackend => ({
	...backend,
	openSession: (options) =>
		Ref.getAndSet(first, false).pipe(
			Effect.flatMap((block) => (block ? Deferred.succeed(opening, undefined).pipe(Effect.andThen(Effect.never)) : backend.openSession(options))),
		),
});

it.live("boot requeues a running spawn without reclaiming its ready berth", () =>
	Effect.gen(function* () {
		...
		const intentId = yield* Effect.gen(function* () {
			...
			expect(Option.getOrThrow(yield* db.Berth.where({ agentId: payload.agentId }).first()).status).toBe("ready");
			expect(Option.getOrThrow(yield* db.Agent.where({ id: payload.agentId }).first()).currentSessionId).toBe(payload.sessionId);
			return submission.id;
		}).pipe(Effect.provide(domainKernelLayer(temporary, backend, {}, recorded.runner)));
		expect(
			yield* Effect.gen(function* () {
				const db = yield* Database;
				return Option.getOrThrow(yield* db.Intent.where({ id: intentId }).first()).status;
		...
		yield* Effect.gen(function* () {
			...
			expect(Option.getOrThrow(yield* db.Agent.where({ id: payload.agentId }).first())).toMatchObject({
				currentSessionId: payload.sessionId,
				status: "alive",
			});
			expect((yield* recorded.provisioned)[1]).toEqual((yield* recorded.provisioned)[0]);
		}).pipe(Effect.provide(domainKernelLayer(temporary, backend, {}, recorded.runner)));
	}),
);
