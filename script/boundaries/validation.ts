...
export const failPolicy = (message: string): never => Effect.runSync(Effect.fail(new BoundaryPolicyInvalid({ message })));

const endpointPath = (endpoint: FixtureEdge["to"]) => (endpoint.kind === "workspace-file" ? endpoint.path : endpoint.name);

const matches = (rule: CompiledBoundaryRule, edge: FixtureEdge) =>
	new RegExp(rule.from.path).test(edge.from.path) && new RegExp(rule.to.path).test(endpointPath(edge.to));
...
const validateExamples = (rule: BoundaryRule, compiled: readonly CompiledBoundaryRule[]) => {
	const illegalMatches = compiled.filter((candidate) => matches(candidate, rule.examples.illegal)).map(({ name }) => name);
	...
	const legalMatches = compiled.filter((candidate) => matches(candidate, rule.examples.legal)).map(({ name }) => name);
	if (legalMatches.length > 0) {
		failPolicy(`Legal example for ${rule.name} must pass every rule: ${describeEdge(rule.examples.legal)} violated ${legalMatches.join(", ")}`);
	}
};
