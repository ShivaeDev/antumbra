...
export const makeSessionTreeLedger = Effect.gen(function* () {
	...
	const refusable = <A>(read: Effect.Effect<A, unknown, never>) =>
		read.pipe(
			Effect.map(Option.some),
			Effect.catchCause((cause) => Effect.logError("the record of a subsession could not be read", cause).pipe(Effect.as(Option.none<A>()))),
		);
	const gapKinds = (sessionId: string) =>
		refusable(db.SessionEvent.where({ kind: GAP, sessionId }).all()).pipe(Effect.map(Option.map((rows) => rows.flatMap(gapKindOf))));
		...
		db.SessionEvent.where({ sessionId })
			...
			.pipe(
				...
				Effect.catchCause((cause) =>
					Effect.logError("a subsession's own provider bytes could not be read back", { sessionId }, cause).pipe(
	...
	const nodeRows = (rootSessionId: string) =>
		db.AgentSession.where({ rootSessionId })
			...
			.pipe(
				...
				Effect.catchCause((cause) =>
					Effect.logError("the nodes of a Session tree could not be read", { rootSessionId }, cause).pipe(
						Effect.as<ReadonlyArray<StoredAgentSession>>([]),
					),
				),
			);
	...
	const nodeRow = (rootSessionId: string, nativeRef: string) =>
		db.AgentSession.where({ nativeRef, rootSessionId })
			.first()
			.pipe(
				Effect.map(Option.flatMap((row) => (isRootSession(row) ? Option.none() : Option.some(row)))),
				Effect.catchCause((cause) =>
					Effect.logError("a subsession row could not be read by its provider reference", { nativeRef, rootSessionId }, cause).pipe(
						Effect.as(Option.none<StoredAgentSession>()),
					),
				),
			);
	const openNodes = db.AgentSession.where(openSessions)
		.where(nodeSessionsOnly)
		.all()
		.pipe(
			Effect.catchCause((cause) =>
				Effect.logError("open subsessions could not be read at startup", cause).pipe(Effect.as<ReadonlyArray<StoredAgentSession>>([])),
			),
		);
	const settle = (sessionId: string, completeness: AgentSessionCompleteness) =>
		db.AgentSession.where({ id: sessionId, completeness: "recording" })
			.update({ completeness })
			.pipe(
				Effect.asVoid,
				Effect.catchCause((cause) => Effect.logError("a subsession's completeness could not be settled", { completeness, sessionId }, cause)),
			);
