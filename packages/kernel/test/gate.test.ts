...
const snapshot = (partial: Partial<AdmissionSnapshot> = {}): AdmissionSnapshot => ({
...
describe("settle", () => {
	...
	it("asks to retry after the remaining quiet time, never sooner than 1ms", () => {
		const gate = settle(100);
		expect(gate.retryAfterMillis?.(snapshot({ millisSinceLastChange: 40 }))).toBe(60);
		expect(gate.retryAfterMillis?.(snapshot({ millisSinceLastChange: 100 }))).toBe(1);
	});
});
...
describe("gaugeCeiling", () => {
	it("admits below the limit, blocks at it, treats a missing reading as zero", () => {
		...
		expect(gate.admits(snapshot({ readings: { "agents.alive": 1 } }))).toBe(true);
		expect(gate.admits(snapshot({ readings: { "agents.alive": 2 } }))).toBe(false);
