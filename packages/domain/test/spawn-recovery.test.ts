...
const until = <E, R>(changes: Stream.Stream<IntentStatus, E, R>, predicate: (status: IntentStatus) => boolean) =>
	changes.pipe(Stream.takeUntil(predicate), Stream.runLast, Effect.map(Option.getOrThrow));
...
it.live("authentication wait survives a full rebuild and retries the same birth", () =>
	Effect.gen(function* () {
		...
		const intentId = yield* Effect.gen(function* () {
			...
			expect(yield* until(submission.changes, (status) => status === "waiting")).toBe("waiting");
			expect(Option.getOrThrow(yield* db.Moorage.where({ agentId: authPayload.agentId }).first()).status).toBe("provisioning");
			expect((yield* db.Berth.where({ agentId: authPayload.agentId }).all()).map((berth) => berth.status)).toEqual(["provisioning"]);
		...
		yield* Effect.gen(function* () {
			...
			expect(yield* until(kernel.changes(intentId), isTerminalIntentStatus)).toBe("succeeded");
			const tried = yield* Ref.get(attempts);
			expect(tried).toHaveLength(2);
			expect(tried[1]).toEqual(tried[0]);
			expect((yield* db.Agent.all()).map((row) => row.id)).toEqual([authPayload.agentId]);
			expect((yield* db.Intent.all()).map((row) => row.id)).toEqual([intentId]);
			expect(Option.getOrThrow(yield* db.Moorage.where({ agentId: authPayload.agentId }).first()).status).toBe("ready");
		}).pipe(Effect.provide(domainKernelLayer(temporary, backend.backend, {}, runner)));
	}),
);
...
it.live("a provision conflict holds the same plan for explicit retry", () =>
	Effect.gen(function* () {
		...
		yield* Effect.gen(function* () {
			...
			expect(yield* until(submission.changes, (status) => status === "waiting" || isTerminalIntentStatus(status))).toBe("waiting");
			const held = Option.getOrThrow(yield* db.Intent.where({ id: submission.id }).first());
			...
			expect(yield* until(kernel.changes(submission.id), isTerminalIntentStatus)).toBe("succeeded");
			const tried = yield* Ref.get(attempts);
			expect(tried).toHaveLength(2);
			expect(tried[1]).toEqual(tried[0]);
		}).pipe(Effect.provide(domainKernelLayer(temporary, backend.backend, {}, runner)));
