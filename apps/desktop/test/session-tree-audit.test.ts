...
const carried = [
	storedLine(SPOKEN, "reading the cluster", AGENT_CALL),
	storedLine(REPORTED, "the cluster maps cleanly", AGENT_CALL),
];
...
const rehearsal = <A, E, R>(
	...
	Effect.gen(function* () {
		...
		yield* use.pipe(
			Effect.provide(rehearsalLayer(temporary, streamRehearsal, stored)),
		);
	});
	...
	() =>
		rehearsal(
			new Map([
				[
					SUBSESSION,
					[...carried, storedLine(UNSEEN, "one more thing", AGENT_CALL)],
				],
			]),
			Effect.gen(function* () {
				const sight = yield* SightSource;
				...
				const gap = (yield* journal(node.id)).find(
					(row) => row.kind === "subsession.gap",
				);
				...
				expect(gap?.payload).toContain(
					"1 of 3 transcript lines the provider stored",
				);
