		...
		<div className="flex min-w-0 shrink-0 flex-col gap-1 border-t border-border px-4 py-2">
			...
			<div className="flex min-w-0 items-end gap-2">
				...
				<Button
					...
					disabled={
						blocked !== undefined || draft.sending || !session?.canAttachImages
					}
					onClick={() => fileInput.current?.click()}
					title={
						session?.canAttachImages
							? "Attach JPEG, PNG, or WebP images"
							: "This backend cannot receive images"
					}
					type="button"
					variant="outline"
				>
			...
			{session?.canAttachImages ? (
				<span className="text-2xs text-muted-foreground">
					Images stay on this device until you send them to {session.backend}.
				</span>
			) : null}
