...
operational meaning two owners. A conduct rule may name the one act that carries it, because the rule is the decision and not the inventory.
Unactionable restatements of process architecture, admission machinery and storage schemas add no decision the Agent can make. Role-specific authority
belongs in the charter; a tool description should not make every Agent read the policies for every other role. The
...
| Open   | The Berth is already on its work branch: never create or switch branches, and open the pull request from that branch with the change tool rather than by hand.           | [Replaceable resources are evidence-bound](../../DESIGN.md#admission-and-resources); a branch Antumbra did not provision can be neither published nor reclaimed. |
...
work calls for it, so a procedure an Agent needs only sometimes stays out of the charter and out of every tool description; the input that needs it
says which skill to use. Each backend hands the same skill files to its harness through the harness's own skill mechanism.
