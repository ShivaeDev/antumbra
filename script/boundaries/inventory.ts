...
export const expectedBoundarySources = (
	root: string,
	sourceRoots: readonly string[],
): readonly string[] =>
	sourceRoots.flatMap((sourceRoot) =>
		walkSources(join(root, sourceRoot), root),
	);

export const boundaryInventoryFailures = (
	...
	if (missing.length > 0) {
		failures.push(
			`dependency-cruiser missed ${missing.length} workspace source(s): ${missing.join(", ")}`,
		);
	}
	return failures;
};
