...
export const useSessionDraft = (sessionId: string, slot: SessionDraftSlot) => {
	const draft = useMemo<SessionDraftRef>(
		() => ({ sessionId, slot }),
		[sessionId, slot],
	);
	const subscribe = useCallback(
		(listener: () => void) => watchSessionDraft(draft, listener),
		[draft],
	);
	...
	const clear = useCallback(
		(sent: SessionDraftSnapshot) => clearUnchangedSessionDraft(draft, sent),
		[draft],
	);
	const setText = useCallback(
		(next: string) => writeSessionDraft(draft, next),
		[draft],
	);
