...
const agentsOf = (
	...
	new Set([
		...world.crews
			...
			.map((crew) => crew.agentId),
		...world.assignments
			.filter((assignment) => pieceIds.has(assignment.pieceId))
			.map((assignment) => assignment.agentId),
	]);

const present = (moments: ReadonlyArray<Date | null>): ReadonlyArray<Date> =>
	moments.flatMap((moment) => (moment === null ? [] : [moment]));
...
export const lastStirredAt = (
	...
	const moments = present([
		...
		...pieces.flatMap((piece) =>
			changesOfPiece(world, piece.id).map((change) => change.activityAt),
		),
	]);
