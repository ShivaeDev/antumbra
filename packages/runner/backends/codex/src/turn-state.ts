import type { BackendFailure, SessionInput } from "@antumbra/runner-ports/backend.ts";
...
export const withPending = (state: OpenTurnState, input: PendingInput): OpenTurnState => ({
	...state,
	pending: [...state.pending, input],
});

export const withTurn = (state: OpenTurnState, turnId: string): OpenTurnState => ({
	...state,
...
export const readyToFlush = (state: OpenTurnState): boolean => Option.isNone(state.turn) && state.pending.length > 0;

export const recordAcceptedTurn = (state: Ref.Ref<TurnState>, turnId: string) =>
	Ref.modify(state, (current) => (current._tag === "closed" ? [false, current] : [true, withTurn(current, turnId)])).pipe(
		Effect.flatMap((open) => (open ? Effect.void : Effect.fail(SESSION_CLOSED))),
	);

export const requireOpen = (state: Ref.Ref<TurnState>) =>
	Ref.get(state).pipe(Effect.flatMap((current) => (current._tag === "closed" ? Effect.fail(SESSION_CLOSED) : Effect.void)));

export const observeTurn = (state: Ref.Ref<TurnState>, turnId: string) =>
	Ref.update(state, (current) => (current._tag === "closed" ? current : withTurn(current, turnId)));

export const closeTurnState = (state: Ref.Ref<TurnState>) => Ref.getAndSet(state, closed);
