...
export interface ToolRegistry {
	readonly call: (threadId: string, name: string, callId: string, args: unknown) => Effect.Effect<Option.Option<DirectToolOutcome>>;
...
const named = (entry: ThreadTools, name: string, callId: string, args: unknown): Effect.Effect<Option.Option<DirectToolOutcome>> =>
	Option.match(Option.fromUndefinedOr(entry.tools.find((tool) => tool.name === name)), {
		onNone: () => Effect.succeed(Option.none()),
		onSome: (tool) => Effect.map(callWhileOpen(entry.calls, tool, callId, args), Option.some),
	});

const served = (map: ByThread, threadId: string, name: string, callId: string, args: unknown): Effect.Effect<Option.Option<DirectToolOutcome>> =>
	Option.match(held(map, threadId), {
		onNone: () => Effect.succeed(Option.none()),
		onSome: (entry) => named(entry, name, callId, args),
	});
...
export const makeToolRegistry: Effect.Effect<ToolRegistry> = Effect.gen(function* () {
	...
	return {
		call: (threadId, name, callId, args) => Effect.flatMap(Ref.get(byThread), (map) => served(map, threadId, name, callId, args)),
