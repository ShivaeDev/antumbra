...
it.live("a Session wakes when its resume attaches, and not before", () =>
	Effect.gen(function* () {
		...
		yield* Effect.gen(function* () {
			...
			expect(yield* untilTerminal(kernel.changes(submission.id))).toBe(
				"succeeded",
			);
...
it.live("a hail of a captain that is answering leaves its work alone", () =>
	Effect.gen(function* () {
		...
		yield* Effect.gen(function* () {
			...
			expect(yield* untilTerminal(kernel.changes(again.intentId))).toBe(
				"succeeded",
			);
			...
			expect(
				(yield* db.AgentSession.where({ agentId: hailed.agentId }).all()).map(
					(row) => [row.id, row.executionStatus],
				),
			).toEqual([[session.id, "active"]]);
