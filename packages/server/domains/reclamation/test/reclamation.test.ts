...
it.app("reclaims retired resources without removing the Agent", function* (app) {
	...
	expect(yield* answered(app.api.reclamation.candidates({}), "the reclamation candidates to be listed")).toEqual([]);
	yield* app.api.agents.retire({ id: agentId });
	expect(yield* answered(app.api.reclamation.candidates({}), "the reclamation candidates to be listed")).toMatchObject([{ agentId }]);
	...
	expect(yield* answered(app.api.reclamation.berths({ agentId }), "the agent's berths to be listed")).toMatchObject([
		{ status: "reclaimed", reclaimState: null, reclaimRequestId: null, strandedAt: null },
	]);
	expect(yield* answered(app.api.agents.byId({ id: agentId }), "the agent to be read")).toMatchObject({ status: "retired" });
});
...
it.app("keeps dirty resources claimed and rejects an obsolete result", function* (app) {
	...
	expect(yield* answered(app.api.reclamation.berths({ agentId }), "the agent's berths to be listed")).toMatchObject([
		{ status: "stranded", reclaimState: "claimed" },
	]);
	...
	expect(yield* answered(app.api.reclamation.berths({ agentId }), "the agent's berths to be listed")).toMatchObject([
		{ status: "stranded", reclaimRequestId: next },
	]);
	yield* app.api.reclamation.reclaimed({ id, claimRequestId: next });
	expect(yield* answered(app.api.reclamation.berths({ agentId }), "the agent's berths to be listed")).toMatchObject([
		{ status: "reclaimed", strandedAt: null },
	]);
});
