...
export const storedRows = [change, pieceChange, changeTransition, changeFeedback] as const;
...
export const storeFeedback = Effect.fn("changes.storeFeedback")(function* (
	rows: WriteHandles<typeof storedRows>,
	items: readonly (typeof changeFeedback.Row.Type)[],
) {
	for (const item of items) {
		if (yield* rows.changeFeedback.exists(item.id)) yield* rows.changeFeedback.update(item.id, item);
		else yield* rows.changeFeedback.insert(item);
	}
});
