...
const berthings = Effect.fnUntraced(function* () {
	...
	for (const [id, voyageId] of [
		["piece-one", "voyage-one"],
		["piece-two", "voyage-two"],
		["piece-empty", "voyage-one"],
	] as const) {
		yield* pieces.charter({ charter: "chart", dependsOn: [], expectation: "charted", id, role: "hand", title: id, voyageId });
	}
});
...
it.effectDB("reads every berthing and resolves only the originating root session", function* (db) {
	yield* Effect.gen(function* () {
		...
		expect(empty.pieces).toEqual([
			...
			{ id: "piece-two", title: "piece-two", voyageName: "voyage-two" },
		...
		expect(reading.rows.filter((row) => row.change.id === "withdrawn").map((row) => row.group)).toEqual(["needsAttention"]);
		expect(reading.rows.filter((row) => row.change.id === "root-change").map((row) => row.originSessionId)).toEqual(["origin"]);
		expect(
