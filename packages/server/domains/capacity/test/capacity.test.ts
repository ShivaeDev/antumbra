...
it.app("keeps a provider blocked until explicit retry", function* (app) {
	...
	expect(yield* answered(app.api.capacity.providers({}), "the capacity providers to be listed")).toEqual([blocked]);
	yield* app.api.capacity.release({ backend: "codex" });
	expect(yield* answered(app.api.capacity.providers({}), "the capacity providers to be listed")).toMatchObject([
		{ backend: "codex", status: "available", reason: null, detail: null, resetsAt: null, utilization: null },
	]);
	yield* app.api.capacity.observe(blocked);
	expect(yield* answered(app.api.capacity.providers({}), "the capacity providers to be listed")).toMatchObject([{ status: "available" }]);
	yield* app.api.capacity.observe({ ...blocked, observedAt: 101 });
	expect(yield* answered(app.api.capacity.providers({}), "the capacity providers to be listed")).toMatchObject([
		{ status: "blocked", observedAt: 101 },
	]);
});
