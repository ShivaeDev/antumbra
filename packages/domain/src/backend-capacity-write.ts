...
export const availableCapacityValues = (
	observedAt: Date,
...
export const ignoreCapacityObservation = (
	...
	const comparison =
		current.value.observedAt.getTime() - observation.observedAt;
	if (
		comparison > 0 ||
		(comparison === 0 &&
			severity(current.value.status) >= severity(observation.status))
	) {
		return true;
	}
...
export const capacityObservationValues = (
	observation: BackendCapacityObservation,
	updatedAt: Date,
) => {
	...
	return {
		...
		resetsAt:
			limited && observation.resetsAt !== undefined
				? new Date(observation.resetsAt)
				: null,
		status: observation.status,
		updatedAt,
		utilization:
			limited && observation.utilization !== undefined
