...
it.live("runs a submitted intent through running to succeeded", () =>
	Effect.gen(function* () {
...
it.live("records a failing intent as failed with its cause in detail", () =>
	Effect.gen(function* () {
		const temporary = yield* acquireTemporaryPersistence;
		const kind = defineIntent({
...
it.live("cancels a queued intent before admission", () =>
	Effect.gen(function* () {
		const temporary = yield* acquireTemporaryPersistence;
...
it.live("interrupts a running intent on cancel", () =>
	Effect.gen(function* () {
		const temporary = yield* acquireTemporaryPersistence;
		const started = yield* Deferred.make<void>();
...
it.live("admits at most the concurrency limit at once", () =>
	Effect.gen(function* () {
		const temporary = yield* acquireTemporaryPersistence;
...
it.live("rejects submits for kinds the kernel was not built with", () =>
	Effect.gen(function* () {
		const temporary = yield* acquireTemporaryPersistence;
...
it.live("holds admission until the system settles, then retries itself", () =>
	Effect.gen(function* () {
		const temporary = yield* acquireTemporaryPersistence;
		const kind = defineIntent({
			execute: () => Effect.void,
