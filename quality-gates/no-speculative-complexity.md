...
1. An abstraction must serve a current caller, boundary, lifetime, or
   composition need. One implementation can still earn that shape, and several
...
4. Generality must be demanded by a current need, not imagined for one. If an
   indirection can be inlined without losing that need, inline it.
