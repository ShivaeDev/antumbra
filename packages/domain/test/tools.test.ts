...
import { acquireTemporaryPersistence, callTool, makeScriptedBackend, type ScriptedBackend, standDown } from "#test/harness.ts";
...
const openedSession = (scripted: ScriptedBackend) =>
	Effect.gen(function* () {
		...
		return live === undefined ? yield* Effect.fail("the session is not scripted") : { agentId: row.agentId, live };
	});
...
it.live("a crew member lands a report against the piece it was spawned for", () =>
	Effect.gen(function* () {
...
it.live("crew write to the board of their piece and of its voyage", () =>
	Effect.gen(function* () {
		...
		yield* Effect.gen(function* () {
			...
			expect(yield* domain.boards.read(BoardScope.Piece({ pieceId: alpha.id }))).toMatchObject([
...
it.live("standing down preserves the agent and session that called it", () =>
	Effect.gen(function* () {
		...
		yield* Effect.gen(function* () {
			...
			const settledSession = Option.getOrThrow(yield* db.AgentSession.where({ id: HAND.sessionId }).first());
