...
export const reconcile = Effect.fn("Mail.reconcile")(function* () {
	...
	const voyages = yield* Effect.forkScoped(Stream.runForEach(live.live(list, {}), () => work.refresh));
	return { refresh: work.refresh, await: Effect.raceAllFirst([work.await, Fiber.join(settings), Fiber.join(voyages)]) };
});
