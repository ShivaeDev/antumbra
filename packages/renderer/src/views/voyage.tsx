...
export const VoyagePanel = ({ onError, voyageId }: { readonly onError: (message: string) => void; readonly voyageId: string }) => {
	const { error, value: voyage } = useFeed<VoyageView>(voyageId, (onVoyage, onFeedError) => watchVoyage(voyageId, onVoyage, onFeedError));

	if (voyage === undefined) {
		return <section className="m-auto text-xs text-muted-foreground">{error === undefined ? "taking a sight…" : `feed lost: ${error}`}</section>;
	}
	return (
		<section className="@container flex min-h-0 min-w-0 flex-1 flex-col font-sans">
			...
			<div className="min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto">
				<div className="grid min-w-0 grid-cols-1 items-start gap-6 px-5 py-4 @4xl:grid-cols-[minmax(0,3fr)_minmax(0,2fr)] @4xl:gap-8">
					<PiecesPanel onError={onError} pieces={voyage.pieces} voyageId={voyage.id} />
					<div className="flex min-w-0 flex-col gap-6">
						<BoardPanel entries={voyage.board} onError={onError} scope={{ kind: "voyage", voyageId: voyage.id }} />
