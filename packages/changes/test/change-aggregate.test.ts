...
it.effectDB(
	"retries the frozen proposal after a lost host response",
	function* () {
		const scripted = yield* makeScriptedHost;
...
it.effectDB(
	"does not transfer a foreign prepared claim during adoption",
	function* (db) {
		const scripted = yield* makeScriptedHost;
		yield* seed;
		yield* Effect.all([
			createPiece("piece-other"),
			createBerth("agent-other", REEF_SOURCE, HEAD),
		]);
		yield* Effect.gen(function* () {
			const changes = yield* Changes;
			...
			expect(
				Option.getOrThrow(yield* db.Change.where({ id: foreign.id }).first()),
			).toEqual(
...
it.effectDB(
	"settles an already-landed host response exactly once",
	function* (db) {
		const scripted = yield* makeScriptedHost;
		const host = {
			...scripted.host,
			open: (request: OpenChangeRequest) =>
				scripted.host
					.open(request)
					.pipe(Effect.map((seen) => ({ ...seen, stage: "landed" as const }))),
		};
		yield* seed;
		yield* Effect.gen(function* () {
			const changes = yield* Changes;
			...
			expect(
				yield* db.ChangeTransition.where({ changeId: landed.id }).all(),
			).toEqual([
