...
const WorkLine = ({ item }: { readonly item: QuayChange }) => {
	...
	return berthing === undefined ? "No linked work" : `${berthing.voyageName} › ${berthing.pieceTitle}`;
};
		...
		<li>
			...
			>
				<span className="flex min-w-0 items-center gap-1.5">
					<span className="min-w-0 flex-1 text-xs font-medium wrap-anywhere">{item.change.title}</span>
				...
				<span className="flex min-w-0 items-center gap-1.5">
					...
					<span className="ml-auto shrink-0 text-2xs text-muted-foreground">{whenLabel(item.change.activityAt)}</span>
				</span>
