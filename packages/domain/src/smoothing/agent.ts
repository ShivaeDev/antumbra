...
export const makeSmootherAgent = Effect.gen(function* () {
	const boards = yield* Boards;
	const db = yield* Database;
	const feeds = yield* DomainFeeds;
	const voyages = yield* Voyages;
	const born = Effect.fnUntraced(function* (voyageId: string) {
	...
	return Effect.fn("Smoothing.smoother")(function* (voyageId: string) {
		const crewed = yield* db.VoyageAgent.where({ role: SMOOTHER_ROLE, voyageId }).first();
		return Option.isSome(crewed) ? crewed.value.agentId : yield* born(voyageId);
	});
});
