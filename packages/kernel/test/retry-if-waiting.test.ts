import { applyMigrations, Database, type PrismaError } from "@antumbra/persistence";
import { acquireTemporaryPersistence, packagedMigrationsDirectory } from "@antumbra/persistence/testing";
import { expect, it } from "@effect/vitest";
import { type Context, Effect, Fiber, Layer, Option, Schema, Stream } from "effect";
...
const replaceDetail = (id: string, detail: string): Effect.Effect<unknown, PrismaError, Context.Service.Identifier<typeof Database>> =>
	Database.use((db) =>
		db.Intent.where({ id })
			.update({ detail })
			.pipe(
				Effect.catchTag("PrismaError", (failure) =>
					transientConnection(failure) ? Effect.yieldNow.pipe(Effect.andThen(replaceDetail(id, detail))) : Effect.fail(failure),
				),
			),
	);
...
it.live("retries only the waiting intent with the expected detail", () =>
	Effect.gen(function* () {
		...
		yield* Effect.gen(function* () {
			...
			const nextTransition = yield* Effect.forkChild(Stream.runHead(kernel.transitions));
			...
			expect(yield* Fiber.join(nextTransition)).toEqual(Option.some({ id: matching.id, status: "queued" }));
...
it.live("loses a retry race safely when the waiting detail changes", () =>
	Effect.gen(function* () {
		...
		yield* Effect.gen(function* () {
			...
			const kernel = yield* Kernel;
			yield* Effect.gen(function* () {
				const retry = yield* Effect.forkScoped(kernel.retryIfWaiting("retry-detail-race", "capacity:held"));
				...
				expect((yield* db.Intent.where({ id: "retry-detail-race" }).first()).pipe(Option.getOrThrow)).toMatchObject({
					detail: "authentication required",
					status: "waiting",
				});
			}).pipe(Effect.ensuring(Effect.sync(() => release.resolve())));
		}).pipe(Effect.provide(KernelLive({ gates: [CLOSED], kinds: [kind] }).pipe(Layer.provideMerge(databaseLayer))));
	}),
);
