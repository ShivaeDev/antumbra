...
describe("artifact windows", () => {
	...
	it("tells an artifact apart from a session wearing the same id", () => {
		...
		expect(subjectOf(artifactPlace("x"))).not.toBe(subjectOf(artifactPlace("y")));
	});
	...
	it.effect("focuses the window an artifact already has", () =>
		Effect.gen(function* () {
			...
			ownWindow(registry, "artifact", artifactPlace("artifact-1"), handleFor(calls, "artifact"));
	...
	it.effect("lets no artifact window open another", () =>
		Effect.gen(function* () {
			...
			const refusal = yield* Effect.flip(asWindow(open(artifactPlace("artifact-2")), shell, "artifact"));
			expect(refusal.reason).toBe("not_the_console");

			const console_ = yield* Effect.flip(asWindow(open(consolePlace), shell, "console"));
	...
	it.effect("never lets an artifact window rename its role", () =>
		Effect.gen(function* () {
			...
			const refusal = yield* Effect.flip(
				asWindow(
					Effect.flatMap(windows, (source) => source.remember(transcriptPlace("session-1"))),
					shell,
					"artifact",
				),
			);
			...
			expect(registry.windowOf("artifact")?.place).toEqual(artifactPlace("artifact-1"));
		}),
	);
