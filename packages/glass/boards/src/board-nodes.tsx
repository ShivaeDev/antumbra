import type { BoardId } from "@antumbra/domain-boards/ids.ts";
...
const summaryTitle = (entry: Entry, name: string): string => {
	if (entry.level === "piece") return `Piece summary · ${name}`;
	return `Day summary · ${entry.createdAt.slice(0, 10)}`;
};
...
const CoveredEntries = (props: NodesProps & { readonly summaryId: string }): ReactNode => {
	const [open, setOpen] = useState(false);
	return (
		<div>
			<button aria-expanded={open} className="cursor-pointer text-2xs text-muted-foreground" onClick={() => setOpen(!open)} type="button">
				Entries behind this summary
			</button>
			{open ? (
				<Live input={{ board: props.board, summaryId: props.summaryId }} query={props.api.boards.under}>
					{(entries) => <BoardNodes {...props} entries={entries} />}
				</Live>
			) : null}
		</div>
	);
	...
	<ul className="flex min-w-0 flex-col gap-3">
		{props.entries.map((entry) => (
			<li className="flex min-w-0 flex-col gap-1" key={entry.id}>
				{entry.kind === "summary" ? <h3 className="text-xs font-medium">{summaryTitle(entry, props.name)}</h3> : null}
				...
				{entry.kind === "summary" ? <CoveredEntries {...props} summaryId={entry.id} /> : null}
			</li>
		))}
	</ul>
);
