...
const failure = (tag: string, detail: string) => new IntentDemandPassFailed({ detail, tag });

const identified = <Payload>(tag: string, identify: (payload: Payload) => string, payload: Payload) =>
	Effect.try({
		catch: (error) => failure(tag, `identity threw: ${String(error)}`),
		try: () => identify(payload),
	}).pipe(
		Effect.flatMap((identity) => (identity.trim().length === 0 ? Effect.fail(failure(tag, "identity must not be empty")) : Effect.succeed(identity))),
	);

export const defineIntentDemand = <Payload, E>({ eligible, identify, kind }: IntentDemandOptions<Payload, E>) => {
	const pass = Effect.gen(function* () {
		const kernel = yield* Kernel;
		const active = yield* kernel.active(kind);
		const activeIdentities = new Set(yield* Effect.forEach(active, ({ payload }) => identified(kind.tag, identify, payload)));
		...
		for (const payload of demanded) {
			...
			if (identities.has(identity)) {
				return yield* failure(kind.tag, `eligible demand identity is duplicated: ${identity}`);
			}
		...
		Effect.catchCause((cause) =>
			Option.match(Cause.findErrorOption(cause), {
				...
				onSome: (error) => (error instanceof IntentDemandPassFailed ? Effect.fail(error) : Effect.fail(failure(kind.tag, String(error)))),
			}),
		),
	);
