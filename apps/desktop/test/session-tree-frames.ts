...
const usage: Extract<SDKMessage, { type: "result" }>["usage"] = {
	cache_creation: {
...
const text = (body: string): ContentBlock => ({
	citations: null,
	text: body,
	type: "text",
});

const task = (id: string, description: string): ContentBlock => ({
	id,
...
const delegating = assistant(
	[task(AGENT_CALL, "Map the session execution cluster")],
	null,
...
const working = assistant(
	[text("reading the cluster")],
	AGENT_CALL,
	"3d4e5f6a-7b8c-4d9e-8f0a-2b3c4d5e6f70",
);

const delegatingAgain = assistant(
	[task(NESTED_CALL, "Read the recovery guide")],
	AGENT_CALL,
	"4e5f6a7b-8c9d-4e0f-9a1b-3c4d5e6f7081",
);
...
const reporting = assistant(
	[text("the cluster maps cleanly")],
	AGENT_CALL,
	"7b8c9d0e-1f2a-4b3c-8d4e-6f7081920314",
);
...
export const rehearsalFrames: ReadonlyArray<SDKMessage> = [
	opened,
	delegating,
	started,
	working,
