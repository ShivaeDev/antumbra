...
import { readLayout, type WindowLayout, writeLayout } from "#adapters/windows/layout.ts";
...
export const fileLayoutStore = (fs: FileSystem.FileSystem, path: string): LayoutStore => ({
	...
	save: (layout) =>
		fs.writeFileString(path, writeLayout(layout)).pipe(Effect.catchCause((cause) => Effect.logWarning("bridge: window layout not saved", cause))),
});
