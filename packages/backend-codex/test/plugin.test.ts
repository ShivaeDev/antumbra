...
const hostFinding = (codex: Option.Option<string>) => makePluginHost({ findExecutable: () => Effect.succeed(codex) });
...
it.effect("the codex plugin registers its backend for the CLI it finds", () =>
	Effect.scoped(
		Effect.gen(function* () {
			...
			yield* Effect.orDie(codexPlugin({ cwd: "/tmp/antumbra", skills: "/tmp/antumbra/skills" }).activate(host.context));
