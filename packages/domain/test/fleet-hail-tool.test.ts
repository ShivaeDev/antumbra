...
import { aliveAgent, eventually, openReefVoyage, seedSpawningCaptain } from "#test/voyage-fixtures.ts";

it.live("the flagship's captain hails a voyage's captain, then reaches it", () =>
	withFlagshipCaptain((captain) =>
		Effect.gen(function* () {
			...
			expect(again.text).toContain(`hailed captain ${agentId} of voyage ${reef.id} — intent `);
			expect((yield* db.Intent.where({ tag: "agent/wake" }).all()).map((intent) => `intent ${intent.id}`)).toContainEqual(
				again.text.slice(again.text.indexOf("intent ")),
			);
			expect(yield* db.VoyageAgent.where({ voyageId: reef.id }).all()).toHaveLength(1);
		}),
	),
);
