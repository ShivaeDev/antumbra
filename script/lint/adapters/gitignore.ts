...
export const insideKept = (
	scope: IgnoreScope,
	directory: string,
): IgnoreScope => ({ boundary: directory, matchers: scope.matchers });

const matcherVerdict = (
	...
	const anchor =
		boundary !== undefined && boundary.length > matcher.base.length
			? boundary
			: matcher.base;
...
export const verdictFor = (
	...
	scope.matchers
		.map((matcher) => matcherVerdict(matcher, scope.boundary, path, directory))
		.findLast((verdict) => verdict !== "unknown") ?? "unknown";
