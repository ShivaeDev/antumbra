import type { adoptionRequest } from "@antumbra/domain-changes/rows/adoption-request.ts";
		...
		{(requests) => (
			<div className="flex flex-col gap-2">
				{requests.map((request) => (
					<AdoptionRequest key={request.id} api={api} request={request} />
				))}
			</div>
		)}
	</Live>
...
const AdoptionRequest = ({ api, request }: { readonly api: ChangesApi; readonly request: typeof adoptionRequest.Row.Type }) => (
