...
const agentIdOf = (segment = ""): string | undefined =>
	segment.startsWith(AGENT) ? segment.slice(AGENT.length) : undefined;
...
export const workflowAgentRef = (
	...
	return agentId === undefined || segments.length < 4
		? undefined
		: { agentId, runId };
};
...
export const subagentRef = (key: SessionKey): string | undefined => {
	const segments = key.subpath?.split("/") ?? [];
	return segments.length >= 2 && segments[0] === SUBAGENTS
		? agentIdOf(segments.at(-1))
		: undefined;
};
...
export const agentFileRef = (agentId: string): string =>
	[SUBAGENTS, `${AGENT}${agentId}`].join("/");
...
export const workflowRunRef = (runId: string): string =>
	[SUBAGENTS, WORKFLOWS, runId].join("/");
