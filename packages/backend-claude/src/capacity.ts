...
const RateLimitType = Schema.Literals([
	"five_hour",
	"seven_day",
	"seven_day_opus",
	"seven_day_sonnet",
...
const decodeRateLimitEvent = Schema.decodeUnknownOption(
	Schema.fromJsonString(RateLimitEvent),
);

const windowLabel = (window: typeof RateLimitType.Type | undefined): string =>
	window === undefined ? "subscription" : window.replaceAll("_", "-");

const limited = (
	info: typeof RateLimitEvent.Type.rate_limit_info,
	status: "blocked" | "warning",
): BackendCapacityClassification => {
	const window = windowLabel(info.rateLimitType);
	return {
		detail:
			status === "blocked"
				? `Claude ${window} usage limit reached`
				: `Claude ${window} usage is approaching its limit`,
		reason: "usage-limit",
...
export const classifyClaudeCapacity = (
	raw: RawPayload,
	...
	return Option.map(
		decodeRateLimitEvent(raw.payload),
		({ rate_limit_info }) => {
			...
			return limited(
				rate_limit_info,
				rate_limit_info.status === "rejected" ? "blocked" : "warning",
			);
		},
	);
};
