...
gap is where the record admits it stopped seeing, and a node's ledger of them is what its completeness is projected from. A node reaches the journal
when the runner first sees it and again whenever what the runner reports about it changes, so a pass that finds nothing new writes nothing. What has
been reported is a claim of the current runner connection and never outlives it, so the first pass after a reconnect reports every node once.
