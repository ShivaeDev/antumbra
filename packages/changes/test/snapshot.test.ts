...
import { changesLayer, createPiece, createRepo, REEF_SOURCE } from "#test/change-harness.ts";

const it = persistenceIt();

it.effectDB("fails its snapshot on invalid stored Change vocabulary", function* (db) {
...
it.effectDB("fails its snapshot on invalid stored PieceChange purpose", function* (db) {
	yield* Effect.all([createRepo("repo-reef", "reef", REEF_SOURCE), createPiece("piece-reef")]);
	yield* db.Change.create(
		changeOf({
			headRef: "work/reef",
			id: "change-valid",
