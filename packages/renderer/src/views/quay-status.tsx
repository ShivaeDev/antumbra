...
export const QuayStatus = ({ item }: { readonly item: QuayChange }) => (
	<section aria-labelledby="quay-status-heading" className="flex flex-col gap-2">
		...
		<div className="grid gap-2 sm:grid-cols-3">
			{changeMarks(item.change).map((mark) => (
				<div className="flex items-center justify-between gap-2 rounded-md border border-border bg-card px-3 py-2" key={mark.key}>
					<span className="text-2xs text-muted-foreground">{mark.key === "merge" ? "Merge" : mark.key}</span>
