...
export const browsableUrl = (raw: unknown): string | undefined => {
	...
	return parsed === null || !BROWSABLE_PROTOCOLS.has(parsed.protocol)
		? undefined
		: parsed.href;
};
...
export const makeOpenExternalHandler =
	...
	(event: DocumentIpcEvent, raw: unknown): void => {
		...
		if (url === undefined) {
			Effect.runFork(
				Effect.logWarning("bridge: refused a link outside http and https", raw),
			);
			return;
		}
		open(url);
	...
	const open = (url: string) => {
		Effect.promise(() => shell.openExternal(url)).pipe(
			Effect.catchCause((cause) =>
				Effect.logError("bridge: external open failed", cause),
			),
			Effect.runFork,
		);
	};
