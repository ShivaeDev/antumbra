...
type ClaimedAttachment = Extract<
	ObservationAttachment,
	{ readonly _tag: "Claimed" }
>;

export const observationConflict = (
...
export const selectMatchedRow = (
	...
	Effect.gen(function* () {
		if (
			Option.isSome(matches.external) &&
		...
		if (
			attachment._tag === "Claimed" &&
			Option.isSome(matches.external) &&
			!matchesClaim(matches.external.value, attachment)
		) {
