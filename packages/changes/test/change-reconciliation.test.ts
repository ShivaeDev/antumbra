...
it.effectDB(
	"attaches a manually landed observation to its prepared Change",
	function* (db) {
...
it.effectDB(
	"attaches an exact later observation and ignores unknown identity",
	function* (db) {
		const scripted = yield* makeScriptedHost;
		yield* seed;
		yield* Effect.gen(function* () {
...
it.effectDB(
	"preserves a prepared replacement when old host identity returns",
	function* (db) {
		const scripted = yield* makeScriptedHost;
		yield* seed;
		yield* Effect.gen(function* () {
...
it.effectDB(
	"keeps the freshest fact within and across observation calls",
	function* (db) {
		const scripted = yield* makeScriptedHost;
		yield* seed;
		yield* Effect.gen(function* () {
			...
			yield* Effect.all(
				[
					changes.observed("scripted", [newest]),
					changes.observed("scripted", [stale]),
				],
				{ concurrency: "unbounded" },
			);
			const stored = Option.getOrThrow(
				yield* db.Change.where({ id: row.id }).first(),
			);
