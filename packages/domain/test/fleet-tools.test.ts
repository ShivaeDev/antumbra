...
it.effectApp("the flagship's captain holds the fleet acts and a captain's own", function* ({ scripted }) {
	const reef = yield* openReefVoyage;
	const { captain: flagship } = yield* flagshipCaptain(scripted);
...
it.effectApp("the flagship's captain reads every voyage in the fleet", function* ({ scripted }) {
	...
	expect(read.text).toContain(`- ${voyageId} Flagship [`);
...
it.effectApp("the flagship's captain reads a voyage it names", function* ({ scripted }) {
...
it.effectApp("a voyage the fleet has not got is refused, not read", function* ({ scripted }) {
	const { captain } = yield* flagshipCaptain(scripted);
...
it.effectApp("a proclamation whose urgency is not a word the fleet knows is refused", function* ({ scripted }) {
	const { captain } = yield* flagshipCaptain(scripted);
	const db = yield* Database;
