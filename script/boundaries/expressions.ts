...
export const compileSelector = (selector: ImportSource | ImportTarget): string => {
	switch (selector.kind) {
		...
		case "all-packages":
			return "^packages/";
		case "any":
			return selector.selectors.map((member) => `(?:${compileSelector(member)})`).join("|");
		case "application":
		...
		case "workspace-except":
			return `^packages/(?!${[...selector.excludedPackages, ...selector.sanctioned.map((exception) => exception.package)]
