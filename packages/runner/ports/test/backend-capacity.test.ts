...
it.effect("publishes fresh availability after a hard block", () =>
	Effect.gen(function* () {
		const capacity = yield* makeBackendCapacityController((evidence) => {
		...
		expect(Option.getOrThrow(yield* capacity.source.current)).toEqual({ observedAt: 41, status: "available" });
