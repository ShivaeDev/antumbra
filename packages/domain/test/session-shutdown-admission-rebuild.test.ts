...
const closeAndHoldSpawn = (
	...
	Effect.gen(function* () {
		...
		const agent = Option.getOrThrow(
			yield* db.Agent.where({ id: WAITING_SPAWN.agentId }).first(),
		);
		expect(agent.status).toBe("spawning");
		const sessionAbsent = Option.isNone(
			yield* db.AgentSession.where({ id: WAITING_SPAWN.sessionId }).first(),
		);
...
const closeAndHoldRecovery = (scripted: ScriptedBackend) =>
	Effect.gen(function* () {
		...
		yield* Effect.yieldNow;
		const session = Option.getOrThrow(
			yield* db.AgentSession.where({ id: recoveryPayload.sessionId }).first(),
		);
...
it.live(
	"rebuild requeues a post-close spawn without writing unattached Session truth",
	() =>
		Effect.gen(function* () {
			...
			const waiting = yield* Effect.promise(() =>
				runtime.runPromise(closeAndHoldSpawn(scripted, recorded)),
			);
			yield* Effect.promise(() => runtime.dispose());
			yield* verifySpawnRebuild(
				waiting.intentId,
				waiting.sessionAbsent,
				scripted,
				recorded,
			).pipe(
				Effect.provide(
...
it.live(
	"rebuild requeues a post-close recovery without waking its Session early",
	() =>
		Effect.gen(function* () {
			const temporary = yield* acquireTemporaryPersistence;
			const scripted = yield* makeScriptedBackend;
			...
			const intentId = yield* Effect.promise(() =>
				runtime.runPromise(closeAndHoldRecovery(scripted)),
			);
			yield* Effect.promise(() => runtime.dispose());
			yield* verifyRecoveryRebuild(intentId, scripted).pipe(
				Effect.provide(
					domainKernelLayer(temporary, backend, {}, recorded.runner),
				),
			);
