...
export const QuayMaster = ({
	...
	const shown = filterQuayChanges(all, filters);
	return (
		<aside
			className={cn(
		...
		>
			<QuayFilterControls filters={filters} onFilters={setFilters} repositories={repositoriesOf(all)} />
			<div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-2">
				<div className="flex items-center px-1 pb-1.5">
					...
					{filtersActive(filters) ? (
						<Button className="ml-auto h-auto px-1 py-0" onClick={() => setFilters(INITIAL_FILTERS)} variant="link">
							Clear filters
						</Button>
					) : null}
				</div>
				...
				{all.length > 0 && shown.length === 0 ? (
					<p className="px-1 py-3 text-xs text-muted-foreground">No pull requests match these filters.</p>
				) : null}
