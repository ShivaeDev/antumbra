...
it.effectApp(
	"stores the choices a request offers in order",
	function* ({ db, rulings }) {
		yield* seedFleet;
...
it.effectApp(
	"stores the rung the asker's request waits on",
	function* ({ rulings }) {
		yield* seedFleet;
...
it.effectApp(
	"stores every subject a request names",
	function* ({ db, rulings }) {
		yield* seedFleet;
		const ruling = yield* rulings.request({
			...asked,
...
it.effectApp(
	"refuses a request naming what the fleet lost",
	function* ({ db, rulings }) {
		yield* seedFleet;
