import { refreshMirror } from "@antumbra/runner-git/mirrors.ts";
import { inspectWorktree } from "@antumbra/runner-git/worktrees.ts";
...
describe("Effect Git", () => {
	it.effect("decodes worktree state from successful process output", () => {
		...
		return Effect.gen(function* () {
			const state = yield* inspectWorktree("/repo").pipe(Effect.provide(gitProcess.pipe(Layer.provide(fake.layer))));
	...
	it.effect("classifies noninteractive credential failure as retryable auth", () => {
		...
		return Effect.gen(function* () {
			const failure = yield* Effect.flip(refreshMirror("/repo").pipe(Effect.provide(gitProcess.pipe(Layer.provide(fake.layer)))));
	...
	it.effect("interrupts a stuck process at the operation deadline", () => {
		...
		return Effect.gen(function* () {
			const fiber = yield* Effect.flip(refreshMirror("/repo").pipe(Effect.provide(gitProcess.pipe(Layer.provide(fake.layer))))).pipe(
				Effect.forkChild,
			);
