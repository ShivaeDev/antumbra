...
import { acquireTemporaryPersistence, callTool, makeScriptedBackend, rawOf, type ScriptedBackend, sessionFor } from "#test/harness.ts";
...
const captainStoodDown = (scripted: ScriptedBackend) =>
	Effect.gen(function* () {
		...
		const session = Option.getOrThrow(Option.fromUndefinedOr((yield* db.AgentSession.where({ agentId: first.agentId }).all())[0]));
		yield* callTool(initial, "stand_down", undefined);
		yield* eventually(
			Effect.gen(function* () {
				expect(Option.getOrThrow(yield* db.AgentSession.where({ id: session.id }).first()).executionStatus).toBe("idle");
			}),
		);
...
const executionOf = (sessionId: string) =>
	Effect.gen(function* () {
		const db = yield* Database;
		return Option.getOrThrow(yield* db.AgentSession.where({ id: sessionId }).first()).executionStatus;
	});

it.live("hailing an idle captain reaches the Session it is already standing in", () =>
	Effect.gen(function* () {
		...
		yield* Effect.gen(function* () {
			const db = yield* Database;
			const domain = yield* AgentDomain;
			const { first, initial, session, voyage } = yield* captainStoodDown(scripted);

			const sleeping = Option.getOrThrow(yield* domain.voyages.read(voyage.id));
