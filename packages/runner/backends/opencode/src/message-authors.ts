...
const modelOf = (info: typeof MessageUpdatedProperties.Type.info, sessionModel: string) =>
	info.providerID === undefined || info.modelID === undefined ? sessionModel : `${info.providerID}/${info.modelID}`;

export const openMessageAuthors = (sessionModel: string): MessageAuthors => {
	...
	return {
		...
		record: (properties) => {
			Option.match(decodeMessage(properties), {
				...
				onSome: ({ info }) => {
					authors.set(info.id, {
						model: modelOf(info, sessionModel),
