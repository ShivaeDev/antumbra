	...
	activate: (context) =>
		Effect.flatMap(
			...
			Option.match({
				...
				onSome: (command) =>
					Effect.flatMap(
						RcRef.make({ acquire: makeOpencodeServer(serveOpencode({ command, cwd: options.cwd, skills: skillFolders(options.skills) })) }),
						(server) => context.registerAgentBackend(opencodeBackend(server)),
					),
			}),
		),
	name: "opencode",
