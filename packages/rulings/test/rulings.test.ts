...
it.effectApp("stores the choices a request offers in order", function* ({
	db,
	rulings,
}) {
	yield* seedFleet;
...
it.effectApp("stores every subject a request names", function* ({
	db,
	rulings,
}) {
	yield* seedFleet;
...
it.effectApp("refuses a request naming what the fleet lost", function* ({
	db,
	rulings,
}) {
	yield* seedFleet;
...
it.effectApp("announces a request once it is written", function* ({ rulings }) {
	yield* seedFleet;
	const feeds = yield* DomainFeeds;
	const notices = yield* feeds.subscribeRulingRefresh();
...
it.effectApp("refuses to read a ruling nothing asked", function* ({ rulings }) {
