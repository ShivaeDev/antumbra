...
} from "@antumbra/platform-vocabulary/session-input.ts";
import { Effect } from "effect";
import sharp, { type Sharp } from "sharp";
import { digestBytes } from "#adapters/inputs/digest.ts";
...
const corrupt = (detail: string) => new InvalidInput({ detail, reason: "corrupt_image" });
...
export const normalizeImage = (bytes: Uint8Array): Effect.Effect<NormalizedImage, InvalidInput> => {
	...
	return Effect.tryPromise({
		catch: (cause) => (cause instanceof InvalidInput ? cause : corrupt("image could not be decoded")),
		try: async () => {
