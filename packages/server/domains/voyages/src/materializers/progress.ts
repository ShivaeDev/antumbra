...
export const voyageProgressProjection = projection("voyageProgress", {
	reads: [voyageActivity, voyage, voyagePieceProgress, voyageCaptainWork],
	writes: [voyageProgress],
	run: Effect.fn("Voyages.projectProgress")(function* (reads, writes) {
		const voyages = yield* reads.voyage.where({});
		const activities = Map.groupBy(yield* reads.voyageActivity.where({}), (activity) => activity.voyageId);
		...
		for (const voyage of voyages) {
			...
			const moments = [
				...pieces.flatMap((piece) => (piece.lastStirredAt === null ? [] : [piece.lastStirredAt])),
				...(activities.get(voyage.id) ?? []).map((activity) => activity.at),
			];
