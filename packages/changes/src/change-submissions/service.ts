...
import type { HeldResource, ResourceReclaimClaimed } from "@antumbra/resource-reclamation";
import type { StoredResourceReclaimStateInvalid } from "@antumbra/vocabulary/agent-runtime";
import { Context, type Effect } from "effect";
import type { ChangeRow } from "#change-rows.ts";
import type { ChangeIdentityCollision, ChangeObservationConflict } from "#change-submissions/errors.ts";
import type {
	AdoptChangeFailure,
	AdoptChangeInput,
	OpenChangeFailure,
...
export class Changes extends Context.Service<
	Changes,
	{
		...
		) => Effect.Effect<ReadonlyMap<string, string>, PrismaError | StoredChangeInvalid | StoredChangeVerdictInvalid | StoredPieceChangeInvalid, never>;
		readonly observed: (
			hostTag: string,
		...
		readonly refresh: (
			...
			| UnknownChangeHostTag
		>;
		readonly snapshot: Effect.Effect<ChangeSnapshot, PrismaError | StoredChangeInvalid | StoredChangeVerdictInvalid | StoredPieceChangeInvalid>;
		readonly submit: (input: SubmitChangeInput) => Effect.Effect<ChangeRow, SubmitChangeFailure>;
		readonly watchable: (hostTag: string) => Effect.Effect<ReadonlyArray<ChangeRow>, PrismaError | StoredChangeInvalid>;
	}
