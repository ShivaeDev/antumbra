...
const layer = SessionEventJournalLive.pipe(Layer.provideMerge(DomainFeedsLive));

it.effectDB("records events in per-Session order", function* (db) {
...
it.effectDB("records native identity before publishing the opening event", function* (db) {
	yield* Effect.gen(function* () {
...
it.effectDB("does not append events when the accompanying write fails", function* (db) {
	yield* Effect.gen(function* () {
		const journal = yield* SessionEventJournal;
