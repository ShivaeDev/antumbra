...
export type ContextDeclaration = ts.InterfaceDeclaration | ts.TypeAliasDeclaration;

export const combineContextStates = (left: ContextState, right: ContextState): ContextState => {
	if (left === true || right === true) return true;
	return left === false || right === false ? false : undefined;
};

const nodeArgumentIsNever = (node: ts.TypeNode, checker: ts.TypeChecker, substitutions: ReadonlyMap<ts.Symbol, ts.TypeNode>): boolean => {
	...
	if (ts.isIntersectionTypeNode(node)) {
		return node.types.some((part) => nodeArgumentIsNever(part, checker, substitutions));
	}
	...
	return replacement !== undefined && nodeArgumentIsNever(replacement, checker, substitutions);
};
...
): boolean => arguments_ === undefined || arguments_.some((argument) => !nodeArgumentIsNever(argument, checker, substitutions));

export const contextDeclaration = (symbol: ts.Symbol): ContextDeclaration | undefined =>
	symbol.declarations?.find(
		(declaration): declaration is ContextDeclaration => ts.isInterfaceDeclaration(declaration) || ts.isTypeAliasDeclaration(declaration),
	);
