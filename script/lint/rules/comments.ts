...
const bodyOf = (comment: SourceComment): string => comment.content.slice(2).trimStart();

const violation = (file: string, line: number, rule: string, message: string): Violation => ({ file, line, message, rule: `comments/${rule}` });

const policyViolations = (file: string, comment: SourceComment, continuation: boolean): readonly Violation[] => {
	...
	if (!body.startsWith("why:") && !(comment.kind === "line" && body.includes("@ts-expect-error")) && !continuation) {
		violations.push(
			violation(file, comment.line, "no-plain-comment", "Comments are banned unless they carry the why: marker or are registered pragmas."),
		);
	}
	return violations;
};

const fileViolations = (file: string, comments: readonly SourceComment[]): readonly Violation[] => {
	const violations: Violation[] = [];
	let continuationLine: number | undefined;
	for (const comment of comments) {
		const continuation = comment.kind === "line" && comment.fullLine && comment.line === continuationLine;
		violations.push(...policyViolations(file, comment, continuation));
		const opensWhyBlock = comment.kind === "line" && comment.fullLine && (bodyOf(comment).startsWith("why:") || continuation);
...
export const commentViolations = (inventory: Inventory): readonly Violation[] =>
	inventory.sources.filter((file) => !isDeclaration(file.path)).flatMap((file) => fileViolations(file.path, file.comments));
