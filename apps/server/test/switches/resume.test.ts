...
it.app("an idle agent is not told to continue its piece while the resume switch is off, and is told when it goes back on", function* (app) {
	...
	const held = yield* eventually(app.api.agents.dispatch({}), (reading) => reading.ready.length === 1);
	...
	const queues = yield* answered(app.api.holds.queues({}));
	...
	const woken = yield* eventually(app.api.sessions.operations({ sessionId }), (operations) => operations.length === 1);
