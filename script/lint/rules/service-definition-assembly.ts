...
const violation = (
	...
	node: ts.Node,
	message: string,
): Violation => ({ file, line: lineOf(source, node), message, rule: RULE });
...
const fileViolations = (file: SourceFile): readonly Violation[] => {
	...
	if (imports.definitions.size === 0 && imports.unsupported.length === 0)
		return [];
	const definitions = directServiceDefinitions(source, imports.definitions);
	const found = imports.unsupported.map((node) =>
		violation(
			file.path,
			source,
			node,
			"Import and call defineService directly.",
		),
	);
	if (imports.definitions.size > 0 && definitions.length === 0) {
		found.push(
			violation(
				...
				source,
				source,
				"Call the imported defineService directly in a top-level service declaration.",
			),
		);
	}
	const allowedBodies = new Set<ts.Node>();
	for (const definition of definitions) {
		for (const problem of serviceDefinitionProblems(
			definition,
			imports.effects,
			imports.values,
			allowedBodies,
	...
	for (const body of implementationBodies(source, allowedBodies)) {
		found.push(
			violation(
				...
				source,
				body,
				"Move this implementation body to a named export in its own focused file.",
			),
		);
	}
	return found;
...
export const serviceDefinitionAssemblyViolations = (
	inventory: Inventory,
): readonly Violation[] => inventory.sources.flatMap(fileViolations);
