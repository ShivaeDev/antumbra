...
const KnownMergeState = Schema.Literals([
	"BEHIND",
	"BLOCKED",
	"CLEAN",
	"DIRTY",
...
const KnownReviewDecision = Schema.Literals([
	"APPROVED",
	"CHANGES_REQUESTED",
	"REVIEW_REQUIRED",
]);
const KnownCheckState = Schema.Literals([
	"ERROR",
	"EXPECTED",
	"FAILURE",
	"PENDING",
...
const unknown = (raw: string): UnknownGitHubWord =>
	UnknownGitHubWord.make({ raw });
const encoded = (word: string | UnknownGitHubWord): string =>
	typeof word === "string" ? word : word.raw;
...
export const GitHubPullState = Schema.String.pipe(
	Schema.decodeTo(Schema.Union([KnownPullState, UnknownGitHubWord]), {
		decode: SchemaGetter.transform((raw) =>
			Schema.is(KnownPullState)(raw) ? raw : unknown(raw),
		),
...
export const GitHubMergeState = Schema.String.pipe(
	Schema.decodeTo(Schema.Union([KnownMergeState, UnknownGitHubWord]), {
		decode: SchemaGetter.transform((raw) =>
			Schema.is(KnownMergeState)(raw) ? raw : unknown(raw),
		),
		encode: SchemaGetter.transform(encoded),
	}),
);
...
export const GitHubReviewDecision = Schema.String.pipe(
	Schema.decodeTo(Schema.Union([KnownReviewDecision, UnknownGitHubWord]), {
		decode: SchemaGetter.transform((raw) =>
			Schema.is(KnownReviewDecision)(raw) ? raw : unknown(raw),
		),
		encode: SchemaGetter.transform(encoded),
	}),
);
...
export const GitHubCheckState = Schema.String.pipe(
	Schema.decodeTo(Schema.Union([KnownCheckState, UnknownGitHubWord]), {
		decode: SchemaGetter.transform((raw) =>
			Schema.is(KnownCheckState)(raw) ? raw : unknown(raw),
		),
		encode: SchemaGetter.transform(encoded),
	}),
);
