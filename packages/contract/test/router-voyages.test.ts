...
import { flagshipSummary, makeRuntime, reefSummary, reefView } from "#fixtures.ts";
...
describe("makeAppRouter, on voyages", () => {
	it.effect("reads verified Artifact Markdown on demand", () =>
		Effect.gen(function* () {
			const artifact = yield* Effect.promise(() => callerOf().artifactMarkdown({ artifactId: "artifact-chart" }));
	...
	it.effect("reads a landed report body on demand", () =>
		Effect.gen(function* () {
			const report = yield* Effect.promise(() => callerOf().reportMarkdown({ reportId: "report-soundings" }));
			expect(report.authorAgentId).toBe("agent-sounder");
			expect(report.markdown).toContain("The eastern shoal is steeper than charted.");
		}),
	);

	it.effect("a report nobody landed surfaces as an error, not an empty body", () =>
		Effect.gen(function* () {
			const outcome = yield* Effect.tryPromise(() => callerOf().reportMarkdown({ reportId: "ghost" })).pipe(Effect.flip);
	...
	it.effect("reads a voyage whole — pieces, crew and board", () =>
		Effect.gen(function* () {
			const read = yield* Effect.promise(() => callerOf().voyage({ voyageId: "voyage-1" }));
			expect(read).toEqual(reefView);
		}),
	);

	it.effect("a voyage nobody opened surfaces as an error, not an empty view", () =>
		Effect.gen(function* () {
			const outcome = yield* Effect.tryPromise(() => callerOf().voyage({ voyageId: "ghost" })).pipe(Effect.flip);
