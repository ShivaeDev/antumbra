...
class ProcessIdentity extends Context.Service<ProcessIdentity, object>()("test/ProcessIdentity") {}
...
it.effect("shares one process service identity across a callback and retains it after startup", () =>
	Effect.gen(function* () {
...
it.effect("finalizes an acquired process runtime exactly once when startup fails", () =>
	Effect.gen(function* () {
		const counted = countedIdentity();
		const runtime = ManagedRuntime.make(counted.layer);
		const startup = ProcessIdentity.pipe(Effect.andThen(Effect.fail("startup failed")));

		const exit = yield* Effect.exit(Effect.promise(() => runManagedRuntimeStartup(runtime, startup)));
...
it.effect("reports the startup failure when runtime disposal also defects", () =>
	Effect.gen(function* () {
		const runtime = ManagedRuntime.make(
			Layer.effect(
				ProcessIdentity,
				Effect.acquireRelease(Effect.succeed({}), () => Effect.die("dispose failed")),
			),
		);
		const exit = yield* Effect.exit(
			Effect.promise(() => runManagedRuntimeStartup(runtime, ProcessIdentity.pipe(Effect.andThen(Effect.fail("startup failed"))))),
		);
