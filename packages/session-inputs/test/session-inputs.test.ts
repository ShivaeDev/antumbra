...
const id = (suffix: string) =>
	SessionInputId.make(`00000000-0000-4000-8000-${suffix.padStart(12, "0")}`);
...
plainIt.effect(
	"rejects corrupt bytes, excessive sources, and non-canonical order",
	() =>
		Effect.gen(function* () {
...
it.effectDB(
	"normalizes, owns, replays, and reads durable images",
	function* (db) {
		...
		yield* Effect.gen(function* () {
			...
			expect(stored.input.parts.map((part) => part.type)).toEqual([
				"image",
				"text",
			]);
			...
			expect(new TextDecoder().decode(thumbnail.bytes.slice(0, 4))).toBe(
				"RIFF",
			);
			expect(new TextDecoder().decode(thumbnail.bytes.slice(8, 12))).toBe(
				"WEBP",
			);
