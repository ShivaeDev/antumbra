...
import { consumeSdkMessages } from "#backends/claude/adapters/session.ts";
...
it.effect("SDK iterator failures reject consumption and refuse input that was never accepted", () =>
	Effect.gen(function* () {
		const input = new InputQueue(() => {});
		const failure = new Error("SDK process failed");
		const failed: AsyncIterable<SDKMessage> = {
			[Symbol.asyncIterator]: () => ({ next: () => Promise.reject(failure) }),
		};
		const receipt = Effect.runPromiseExit(input.push(message("not accepted")));
		const consumption = consumeSdkMessages(failed, input, () => {});
		yield* Effect.promise(() => expect(consumption).rejects.toBe(failure));
		expect(Exit.isFailure(yield* Effect.promise(() => receipt))).toBe(true);
	}),
);
