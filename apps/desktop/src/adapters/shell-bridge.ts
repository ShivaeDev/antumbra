...
export const registerShellBridge = <R, E>(
	runtime: ManagedRuntime.ManagedRuntime<ServerProcess | ShellDrafts | WindowSource | R, E>,
	...
	const run = <A, F>(effect: Effect.Effect<A, F, ServerProcess | ShellDrafts | WindowSource>) => runtime.runPromise(effect);
	...
	ipcMain.handle(Channel.CLEAR_DRAFT_CHANNEL, (event, raw: unknown, revision: unknown) => {
		owner(event);
		return run(ShellDrafts.use((source) => source.clear(Schema.decodeUnknownSync(DraftRef)(raw), Schema.decodeUnknownSync(Schema.String)(revision))));
	});
	return run(
		ShellDrafts.use((source) =>
			Effect.sync(() =>
