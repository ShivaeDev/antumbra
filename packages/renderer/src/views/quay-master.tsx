...
const filtersActive = (filters: QuayFilters): boolean =>
	filters.query !== "" ||
...
const ChangeList = ({
	...
	<nav aria-label="Pull requests">
		<ul className="flex flex-col gap-1">
			{shown.map((item) => (
				<QuayListRow
					current={item.change.id === selectedId}
					item={item}
					key={item.change.id}
					onSelect={onSelect}
				/>
		...
		<div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-2">
			<div className="flex items-center px-1 pb-1.5">
				...
				{filtersActive(filters) ? (
					<Button
						...
						onClick={() =>
							onFilters({ query: "", repositoryId: null, status: "all" })
						}
						variant="link"
					>
						Clear filters
			...
			{all.length === 0 ? (
				<p className="px-1 py-3 text-xs text-muted-foreground">
					Nothing at the quay — a change appears once a piece opens one, or once
					you adopt one by hand.
				</p>
			) : null}
			...
			{shown.length === 0 ? null : (
				<ChangeList onSelect={onSelect} selectedId={selectedId} shown={shown} />
			)}
		</div>
	</aside>
);
