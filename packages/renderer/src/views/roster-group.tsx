...
export const RosterGroupPanel = ({
	...
	<section className="flex min-w-0 flex-col gap-2">
		<header className="flex items-center gap-2">
			<span className={cn("size-1.5 shrink-0 rounded-full", DOT[group.standing])} />
			<h3 className="text-2xs font-medium text-muted-foreground">{STANDING_LABEL[group.standing]}</h3>
			<span className="text-2xs text-muted-foreground">{group.agents.length}</span>
