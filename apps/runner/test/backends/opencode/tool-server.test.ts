import { servedTools } from "@antumbra/runner-backends-opencode/session-tools.ts";
import { makeToolSessions, type ToolSessions } from "@antumbra/runner-backends-opencode/tool-sessions.ts";
...
const tool = (name: string, said: string): DirectTool => ({
	call: (_callId, args) => Effect.succeed({ ok: true, text: `${said} ${JSON.stringify(args)}` }),
		...
		Effect.flatMap((response) =>
			Effect.promise(() => response.text()).pipe(
				Effect.map((text): unknown => {
					...
					return JSON.parse(data?.slice(6) ?? text);
				}),
			),
		),
	);

const called = (url: string, name: string, args: Record<string, unknown>) =>
	asked(url, "tools/call", { arguments: { callerCall: "call_1", ...args }, name });
...
it.effect("preserves the provider call id and strips caller identity before invoking the bound tool", () =>
	Effect.scoped(
		Effect.gen(function* () {
			const calls: Array<readonly [string, unknown]> = [];
			const direct: DirectTool = {
				...tool("read_board", "read"),
				call: (callId, args) =>
					Effect.sync(() => {
			...
			const sessions = makeToolSessions([direct.name]);
			yield* remember(sessions, "ses_crew", [direct]);
			const url = yield* listening(sessions);
			yield* called(url, direct.name, { callerCall: "provider_call", callerSession: "ses_crew", since: 3 });
			expect(calls).toEqual([["provider_call", { since: 3 }]]);
		}),
	),
);
