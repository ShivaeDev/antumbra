...
const decodeCount = (output: string) => Schema.decodeUnknownEffect(CommitCount)(output.trim());

const countUnpushedCommits = (repository: string, revision: string, operation: "inspect-branch" | "inspect-worktree") =>
	Effect.gen(function* () {
...
export const countUnpushedBranchCommits = (
	mirror: string,
	branch: string,
): Effect.Effect<number, GitError, ChildProcessSpawner.ChildProcessSpawner> => countUnpushedCommits(mirror, branch, "inspect-branch");
...
export const removeWorktree = (mirror: string, path: string): Effect.Effect<void, GitError, ChildProcessSpawner.ChildProcessSpawner> =>
	runGit({
...
export const pruneWorktrees = (mirror: string): Effect.Effect<void, GitError, ChildProcessSpawner.ChildProcessSpawner> =>
	runGit({
...
export const deleteBranch = (mirror: string, branch: string): Effect.Effect<void, GitError, ChildProcessSpawner.ChildProcessSpawner> =>
	runGit({
