...
it.app("updates a registered source without changing its identity or position", function* (app) {
	...
	const first = yield* answered(app.api.repos.byId({ id: reef }), "the reef repo to be read");
	...
	yield* app.settle();
	const stored = yield* answered(app.api.repos.all({}), "all repos to be listed");
	...
	expect(yield* answered(app.api.repos.byIds({ ids: [shoal, reef] }), "the repos to be read")).toEqual(stored);
});
...
it.app("refuses sources whose berth slug is already registered", function* (app) {
	...
	expect(yield* answered(app.api.repos.all({}), "all repos to be listed")).toMatchObject([{ source: "/reefs/Reef-Charts", defaultRef: "main" }]);
});
...
it.app("forgets one registration and leaves another available", function* (app) {
	...
	yield* app.api.repos.forget({ id: reef });
	expect(yield* answered(app.api.repos.byId({ id: reef }), "the reef repo to be read")).toBeNull();
	expect(yield* answered(app.api.repos.all({}), "all repos to be listed")).toMatchObject([{ id: shoal, source: "/reefs/two" }]);
});

it.app("keeps the registration boundary's raw string inputs", function* (app) {
	yield* app.api.repos.register({ source: "", defaultRef: "" });
	expect(yield* answered(app.api.repos.all({}), "all repos to be listed")).toMatchObject([{ name: "repo", source: "", defaultRef: "" }]);
});
