...
export const withSqlite = <A>(
	path: string,
	act: (database: DatabaseSync) => A,
): A => {
...
export const seedDatedOpenSession = (
	database: DatabaseSync,
	agentId: string,
	id: string,
	createdAt: string,
	...
	database
		.prepare(
			'INSERT INTO "agentSession" ("id", "agentId", "backend", "cwd", "status", "executionStatus", "createdAt") VALUES (?, ?, ?, ?, ?, ?, ?)',
		)
		.run(id, agentId, "scripted", `/tmp/${agentId}`, "open", "idle", createdAt);

export const pointAgentAt = (
	database: DatabaseSync,
	agentId: string,
	sessionId: string,
) =>
	database
		.prepare('UPDATE "agent" SET "currentSessionId" = ? WHERE "id" = ?')
...
export const sessionsOf = (
	...
	withSqlite(database, (sqlite) =>
		sqlite
			.prepare(
				'SELECT "id", "status", "completeness" FROM "agentSession" WHERE "agentId" = ? ORDER BY "id"',
			)
			.all(agentId),
	);

export const oneOpenRootIndexExists = (
	database: TemporaryPersistence["database"],
) =>
	withSqlite(database, (sqlite) =>
		sqlite
			.prepare(
				`SELECT COUNT(*) AS "count" FROM "sqlite_master" WHERE "type" = 'index' AND "name" = 'agentSession_one_open_root_per_agent'`,
			)
			.get(),
	);
