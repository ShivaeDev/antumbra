...
const vocabularyRule =
	boundaryPolicy.find(
		(
			rule,
		): rule is Extract<BoundaryRule, { readonly kind: "vocabulary-access" }> =>
			rule.kind === "vocabulary-access",
...
describe("boundary policy inventory", () => {
	it("configures dependency-cruiser to resolve package exports", () => {
		expect(
			compiledBoundaryPolicy.configuration.options.enhancedResolveOptions,
		).toEqual({ conditionNames: ["import"], exportsFields: ["exports"] });
	});

	it("rejects unknown and empty authored workspace selectors", () => {
		expect(() =>
			compileBoundaryPolicy(
				[withConsumers(packages.named("not-a-package"))],
				boundaryPolicyInventory,
			),
		).toThrow("names unknown package not-a-package");
		expect(() =>
			compileBoundaryPolicy(
				[withConsumers(packages.named())],
				boundaryPolicyInventory,
			),
		).toThrow("names no package units");
		expect(() =>
			compileBoundaryPolicy([withConsumers(anyOf())], boundaryPolicyInventory),
		).toThrow("has an empty category");
		expect(() =>
			compileBoundaryPolicy(
				[withConsumers(packages.inFamily("not-a-family"))],
				boundaryPolicyInventory,
			),
	...
	it("rejects unknown vocabulary subjects", () => {
		expect(() =>
			compileBoundaryPolicy(
				[{ ...vocabularyRule, allowedSubjects: ["not-a-subject"] }],
				boundaryPolicyInventory,
			),
		).toThrow("names unknown vocabulary subject not-a-subject");
	});
});
