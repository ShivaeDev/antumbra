...
const immutableDeclaration = (node: ts.VariableDeclaration): boolean =>
	(node.parent.flags & ts.NodeFlags.Const) !== 0;

const importedEffectValue = (
	...
	if (ts.isImportSpecifier(declaration)) {
		...
		if (
			imported === "Context" &&
			(module === "effect" || module === "effect/Context")
		) {
			return "Context";
		}
		return module === "effect/Context" &&
			(imported === "Service" || imported === "Tag")
			? imported
			: undefined;
	}
...
const symbolEffectValue = (
	...
	return ts.isBindingElement(declaration)
		? bindingValue(declaration, checker, seen)
		: undefined;
};

const propertyEffectValue = (
	expression: ts.PropertyAccessExpression,
	checker: ts.TypeChecker,
	seen: Set<ts.Symbol>,
): EffectValue | undefined => {
	const selected = symbolEffectValue(
	...
	if (
		owner === "Context" &&
		(expression.name.text === "Service" || expression.name.text === "Tag")
	) {
...
export function effectValue(
	...
	return ts.isIdentifier(current)
		? identifierEffectValue(current, checker, seen)
		: undefined;
}
