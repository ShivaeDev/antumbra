...
const subjectsOf = (
	...
	...Option.toArray(reader.pieceId).map(
		(id): RulingSubject => ({ id, kind: "piece" }),
	),
	...Option.toArray(reader.voyageId).map(
		(id): RulingSubject => ({ id, kind: "voyage" }),
	),
...
export const standingRulingsFor = Effect.fn("domain.standingRulingsFor")(
	function* (reader: RulingReader, tags: ReadonlyArray<string> = []) {
		...
		return ruled.filter(
			(ruling) => ruling.radius === "fleet" || bound.has(ruling.id),
		);
	},
);
...
const chosen = (ruling: Ruling, answer: RulingAnswer): string => {
	const picked = Option.flatMap(answer.choiceId, (choiceId) =>
