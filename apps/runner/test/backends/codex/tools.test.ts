...
const landReport = (calls: Ref.Ref<ReadonlyArray<unknown>>): DirectTool => ({
	call: (callId, args) => Ref.update(calls, (all) => [...all, { callId, args }]).pipe(Effect.as({ ok: true, text: "report landed" })),
...
it.live("a tool call runs the tool and answers with its outcome", () =>
	Effect.gen(function* () {
		...
		expect(yield* Ref.get(calls)).toEqual([{ callId: "call-1", args: { title: "soundings" } }]);
	}),
);
