...
import { BirthId, identity } from "@antumbra/domain-agents/ids.ts";
...
it.app("provision refusal holds the birth and explicit retry reuses its prepared resources", function* (app) {
	...
	const id = BirthId.make(requested);
	...
	yield* app.api.agents.retry({ requestId: Request.make("retry"), id });
	...
	expect(start).toMatchObject({
		...
		charter: { id: "birth:charter", parts: [{ type: "text", text: expect.stringContaining("Standing orders") }] },
	});
...
it.app("raising the running budget admits the next held birth", function* (app) {
	...
	expect((yield* eventually(app.api.agents.admitted({}), (births) => births.length === 1)).map((birth) => birth.id)).toEqual(["first"]);
	yield* app.api.settings.setCount({ key: "maxParallelSessions", count: 2 });
	expect((yield* eventually(app.api.agents.admitted({}), (births) => births.length === 2)).map((birth) => birth.id)).toEqual(["first", "second"]);
});
