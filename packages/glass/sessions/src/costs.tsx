...
export const CostsPanel = ({ api }: { readonly api: SessionsApi }) => (
	<Live
		query={api.costs.reading}
		input={{ today: new Date(Effect.runSync(Clock.currentTimeMillis)).toISOString().slice(0, 10) }}
		waiting="Reading costs…"
	>
		{(costs) => (
