...
const Stale = ({ stale, standing }: { readonly stale: ReadonlyArray<StandingRulingView>; readonly standing: ReadonlyArray<StandingRulingView> }) =>
	stale.length === 0 ? null : (
		<section className="flex min-w-0 flex-col border-t border-border">
			...
			<StandingRulingList listed={stale} standing={standing} />
		</section>
	);

const Standing = ({ error, standing }: { readonly error: string | undefined; readonly standing: StandingRulingsView | undefined }) => {
	...
	return (
		<>
			{binding.length === 0 ? null : <StandingRulingList listed={binding} standing={standing.rulings} />}
			<Stale stale={standing.rulings.filter((ruling) => ruling.stale)} standing={standing.rulings} />
		</>
	);
};

export const StandingRulings = ({ error, standing }: { readonly error: string | undefined; readonly standing: StandingRulingsView | undefined }) => (
	<section className="flex min-w-0 flex-col border-t border-border">
		<header className="flex flex-col gap-1 px-4 pt-3 pb-2">
		...
		<Standing error={error} standing={standing} />
	</section>
);
