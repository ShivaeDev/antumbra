...
// unrepresentable.
it.effect(
	"closes every open root but the newest when an Agent points nowhere",
	() =>
		Effect.gen(function* () {
...
it.effect("keeps the Session an Agent points at and closes newer history", () =>
	Effect.gen(function* () {
		...
		expect(
			withSqlite(database, (sqlite) =>
				sqlite
					.prepare('SELECT "currentSessionId" FROM "agent" WHERE "id" = ?')
					.get("agent-explicit"),
			),
		).toEqual({ currentSessionId: "session-held" });
	}),
);
