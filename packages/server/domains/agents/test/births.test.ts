...
it.app("births reserve the global running budget oldest first", function* (app) {
	...
	expect((yield* eventually(app.api.agents.admitted({}), (births) => births.length === 1)).map((held) => held.id)).toEqual(["one"]);
	expect(
		yield* Effect.flip(
			app.api.agents.admit({ id: BirthId.make("two"), backend: "claude", model: null, effort: null, requestId: Id.Request.make("admit-two") }),
		),
...
it.app("cancelling an unadmitted birth removes its demand without retiring the identity", function* (app) {
	...
	yield* app.api.agents.spawn(asking("one"));
	yield* app.api.agents.cancel({ id: BirthId.make("one"), requestId: Id.Request.make("cancel") });
...
it.app("failed start waits and explicit retry has a new deduplicated edge request", function* (app) {
	...
	yield* app.api.agents.retry({ id: BirthId.make("one"), requestId: Id.Request.make("retry") });
...
it.app("smoothing reuses its Agent across fresh constrained sessions", function* (app) {
	...
	yield* app.api.agents.cancel({ id: BirthId.make("smooth-two"), requestId: Id.Request.make("cancel-pass") });
