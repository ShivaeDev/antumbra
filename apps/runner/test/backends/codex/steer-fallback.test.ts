...
import { turnRequests } from "@antumbra/runner-backends-codex/turn-requests.ts";
import { makeTurnDriver } from "@antumbra/runner-backends-codex/turns.ts";
...
it.live("interrupt accepts an inactive turn or a timeout while draining", () =>
	Effect.gen(function* () {
		...
		for (const detail of ["no active turn", "timeout waiting for turn/interrupt"]) {
			const requests = turnRequests({ ...server, request: () => Effect.fail(codexFailure(detail)) }, "thread-1", { model: "gpt-5-codex" });
