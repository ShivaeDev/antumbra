...
const writes = [session, sessionOperation, sessionToolCall, sessionStartResult] as const;
type Rows = WriteHandles<typeof writes>;
type Observation = FactPayload<typeof observed> & { at: number; requestId: string };
type Session = typeof session.Row.Type;
const start = Effect.fn("sessions.start")(function* (fact: Observation, rows: Rows) {
...
const tree = Effect.fn("sessions.tree")(function* (fact: Observation, rows: Rows, current: Session, nodes: readonly Session[]) {
	const evidence = fact.evidence;
	const at = new Date(fact.at).toISOString();
	...
	if (evidence.type === "opened") {
		...
		if (existing !== undefined) {
			...
			yield* rows.session.insert({
				...current,
				id: SessionId.make(`${current.rootSessionId}:${evidence.nativeRef}`),
				nativeRef: evidence.nativeRef,
...
const activity = Effect.fn("sessions.activity")(function* (fact: Observation, rows: Rows, current: Session, nodes: readonly Session[]) {
	const evidence = fact.evidence;
	const at = new Date(fact.at).toISOString();
...
const tools = Effect.fn("sessions.tools")(function* (fact: Observation, rows: Rows, current: Session) {
	const evidence = fact.evidence;
	const at = new Date(fact.at).toISOString();
...
const settle = Effect.fn("sessions.settle")(function* (fact: Observation, rows: Rows, current: Session) {
	const evidence = fact.evidence;
	const at = new Date(fact.at).toISOString();
