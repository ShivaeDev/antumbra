...
const roleOn = (world: VoyageWorld, identity: SessionIdentity): string =>
	Option.match(identity.voyageId, {
		onNone: () => "",
		onSome: (voyageId) =>
			crewOf(world, voyageId).find(
				(member) => member.agentId === identity.agentId,
			)?.role ?? "",
	});
...
export const rulesAs = (
	world: VoyageWorld,
	identity: SessionIdentity,
): RulingAuthority =>
	isFlagshipCaptainIdentity(world, roleOn(world, identity), identity)
		? "flagship"
		: "captain";
