...
const windows = Effect.flatMap(WindowSource, (source) => Effect.succeed(source));
...
describe("window source", () => {
	...
	it.effect("lets only the console open windows, and never another console", () =>
		Effect.gen(function* () {
			const registry = makeWindowRegistry();
			const shell = { document: "file:///app/console.html", registry };
			ownWindow(registry, "console", consolePlace);
			...
			expect((yield* Effect.flip(asWindow(open(transcriptPlace("session-2")), shell, "child"))).reason).toBe("not_the_console");
			expect((yield* Effect.flip(asWindow(open(consolePlace), shell, "console"))).reason).toBe("console_is_not_a_target");
			expect(registry.children()).toHaveLength(1);
		}),
	);

	it.effect("focuses the window a subject already has instead of minting one", () =>
		Effect.gen(function* () {
			const registry = makeWindowRegistry();
			const shell = { document: "file:///app/console.html", registry };
			...
			yield* asWindow(
				Effect.flatMap(windows, (source) => source.open(transcriptPlace("session-1"))),
				shell,
				"console",
			);
	...
	it.effect("lets a window move within its role but never rename it", () =>
		Effect.gen(function* () {
			...
			const shell = { document: "file:///app/console.html", registry };
			ownWindow(registry, "console", consolePlace);
			const remember = (place: WindowPlace) => Effect.flatMap(windows, (source) => source.remember(place));

			yield* asWindow(remember({ ...consolePlace, mode: "quay", voyageId: "voyage-1" }), shell, "console");
			...
			expect((yield* Effect.flip(asWindow(remember(transcriptPlace("session-1")), shell, "console"))).reason).toBe("role_is_immutable");
