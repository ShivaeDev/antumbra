...
import { cloneMirror, refreshMirror } from "@antumbra/runner-git/mirrors.ts";
import { pushBranch } from "@antumbra/runner-git/push.ts";
import { addWorktree, inspectWorktree } from "@antumbra/runner-git/worktrees.ts";
...
describe("pushing a work branch", () => {
	it.live("moves one branch and leaves the trunk alone", () =>
		Effect.gen(function* () {
			...
			yield* pushBranch(worktree, BRANCH, sha(worktree, "HEAD"));
			expect(sha(remote, `refs/heads/${BRANCH}`)).toBe(sha(worktree, "HEAD"));
		}).pipe(Effect.provide(gitProcess.pipe(Layer.provide(NodeServices.layer)))),
	);
	...
	it.effect("refuses any branch outside work/ without spawning git", () =>
		...
		}).pipe(Effect.provide(gitProcess.pipe(Layer.provide(forbiddenSpawner)))),
	);
});
