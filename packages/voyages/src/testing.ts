...
export type ScriptedSailing = Ref.Ref<Sailing>;

export const scriptedSailing = (): ScriptedSailing => Ref.makeUnsafe<Sailing>(new Map());
...
export const scriptedVoyagesOn = (state: ScriptedSailing): Layer.Layer<Voyages, never, Context.Service.Identifier<typeof Database> | RoleSettings> =>
		...
		Effect.gen(function* () {
			...
			return {
				...
				ensureFlagship: Effect.fnUntraced(function* (input: OpenVoyageInput) {
					...
					for (const voyage of sailing.values()) {
...
export const scriptedVoyages: Layer.Layer<Voyages, never, Context.Service.Identifier<typeof Database> | RoleSettings> = Layer.unwrap(
	Effect.sync(() => scriptedVoyagesOn(scriptedSailing())),
);
