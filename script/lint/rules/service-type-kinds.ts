...
const declarationType = (symbol: ts.Symbol): ts.TypeNode | undefined =>
	symbol.declarations?.find(ts.isTypeAliasDeclaration)?.type;

export const nodeIsComputation = (
	node: ts.TypeNode,
	...
	return (
		declaration !== undefined && nodeIsComputation(declaration, checker, seen)
	);
};

export const typeIsComputation = (
	type: ts.Type,
	checker: ts.TypeChecker,
): boolean => {
	...
	return (
		...
		nodeIsComputation(declaration, checker, new Set([symbol]))
	);
};
