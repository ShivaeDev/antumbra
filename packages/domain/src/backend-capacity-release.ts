	...
	const reconcile = serial.withPermit(
		Effect.gen(function* () {
			const statuses = new Map(
				(yield* domain.backendCapacities.snapshot).map((capacity) => [
					capacity.backend,
					capacity.status,
				]),
			);
			...
			const parked = [...spawns, ...wakes].flatMap((intent) => {
				...
				return Option.isSome(hold) &&
					statuses.get(hold.value.backend) === "available"
					? [{ detail: intent.detail, id: intent.id }]
					: [];
			});
			yield* Effect.forEach(
				parked,
				(intent) => kernel.retryIfWaiting(intent.id, intent.detail),
				{ concurrency: 1, discard: true },
			);
		}),
	);
	const release = (backend: string) =>
		domain.backendCapacities
			.clear(backend)
			.pipe(
				Effect.andThen(reconcile),
				Effect.andThen(domain.backendCapacities.announce),
			);
...
export const BackendCapacityReleaseLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const guarded = releases.reconcile.pipe(
			Effect.catchCause((cause) =>
				Effect.logError(
					"backend capacity release reconciliation failed",
					cause,
				),
			),
		);
