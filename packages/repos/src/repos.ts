...
export class Repos extends Context.Service<Repos, RepoRegistry>()("@antumbra/repos/Repos") {}
...
export const ReposLive = Layer.effect(Repos)(
	Effect.gen(function* () {
		...
		const context = Context.make(Database, db).pipe(Context.add(DomainFeeds, feeds));
		return {
			forget: (id) => Effect.provide(forgetRepo(id), context),
			list: Effect.provide(listRepos, context),
			register: (registration) => registrationGate.withPermits(1)(Effect.provide(registerRepo(registration), context)),
		};
	}),
);
