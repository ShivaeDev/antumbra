...
it.app("clears the listing failure after a successful refresh", function* (app) {
	...
	expect(yield* answered(catalogue.catalog({ backend: "opencode" }), "the backend's catalog to be read")).toEqual({
		backend: "opencode",
...
it.app("has no failure before the first listing", function* (app) {
	expect(yield* answered(app.api.backends.catalog({ backend: "opencode" }), "the backend's catalog to be read")).toEqual({
		backend: "opencode",
		failure: null,
		imageInput: null,
	});
