...
const nothingStanding = (view: { readonly rulings: ReadonlyArray<unknown> }) =>
	view.rulings.length === 0;

it.effectDB(
	"a withdrawal drops the ruling from the standing feed",
	function* () {
...
it.effectDB(
	"refuses a withdrawal the record refuses, in its words",
	function* () {
		yield* Effect.gen(function* () {
			yield* seedFleet;
			const rulings = yield* Rulings;
			...
			const unruled = yield* Effect.flip(
				source.withdraw({ note: "never mind", rulingId: open.id }),
			);
			const again = yield* Effect.flip(
				source.withdraw({ note: "once more", rulingId: standing.id }),
			);
			const adrift = yield* Effect.flip(
				source.withdraw({ note: "no such rule", rulingId: "ruling-adrift" }),
			);
...
it.effectDB("refuses to supersede with a withdrawn ruling", function* () {
	yield* Effect.gen(function* () {
		...
		const refused = yield* Effect.flip(
			source.supersede({ byRulingId: retired.id, rulingId: older.id }),
		);
